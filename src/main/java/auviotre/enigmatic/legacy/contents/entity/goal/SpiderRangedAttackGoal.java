...
public class SpiderRangedAttackGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        if (mobTarget != null && EnigmaticHandler.isCurseBoosted(this.spider) && mobTarget.isAlive()) {
    ...
    public void tick() {
        double distance = 0;
        if (this.target != null) {
            distance = this.spider.distanceToSqr(this.target.getX(), this.target.getY(), this.target.getZ());
        }
