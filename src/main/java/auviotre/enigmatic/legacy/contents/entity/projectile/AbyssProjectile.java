...
public class AbyssProjectile extends Projectile implements ItemSupplier {
    ...
    public AbyssProjectile(Level world, LivingEntity owner) {
        this(EnigmaticEntities.ABYSS_PROJECTILE.get(), world);
        this.setOwner(owner);
    }

    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        builder.define(ID_DAMAGE, 0F);
    }

    public void setDamage(float damage) {
        this.entityData.set(ID_DAMAGE, damage);
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide()) {
            ...
            if (cachedTarget != null && cachedTarget.isAlive()) {
                Vec3 target = new Vec3(cachedTarget.getX(), cachedTarget.getY(0.16F), cachedTarget.getZ());
                Vec3 delta = target.subtract(this.position());
                if (delta.length() < 24F) {
                    delta = delta.normalize().scale(0.4F).add(movement);
                    movement = delta.scale(0.8F);
...
//                this.level().addParticle(EnigmaticAddonParticles.ABYSS_CHAOS, x, y, z, 0, 0, 0);
    ...
    protected void onHitEntity(EntityHitResult hitResult) {
        ...
        if (entity instanceof LivingEntity livingEntity) {
            ...
            float damage = this.entityData.get(ID_DAMAGE) * 0.3F;
            if (target instanceof LivingEntity living) damage += living.getMaxHealth() * 0.02F * level;
            target.hurt(EnigmaticDamageTypes.source(this.level(), EnigmaticDamageTypes.ABYSS, this, this.getOwner()), damage);
