...
public class GhastMultishotGoal extends Goal {
    ...
    public boolean canUse() {
        return EnigmaticHandler.isCurseBoosted(this.ghast) && this.ghast.getTarget() != null;
    }
            ...
            if (target.distanceToSqr(this.ghast) < 4096.0 && this.ghast.hasLineOfSight(target)) {
                ...
                if (this.chargeTime == 20) {
                    ...
                    if (difficulty > 0) {
                        ...
                        for (int i = 0; i < num; i++) {
                            Vec3 delta = new Vec3(randomV(0.2F), randomV(0.2F), randomV(0.2F));
                            Vec3 F = new Vec3(x, y, z).normalize().scale(0.25F);
                            SmallFireball smallFireball = new SmallFireball(level, this.ghast, F);
