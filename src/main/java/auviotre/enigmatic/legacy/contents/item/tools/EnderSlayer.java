...
public class EnderSlayer extends SwordItem implements ICursed {
    ...
    public EnderSlayer() {
        super(TIER, new Item.Properties().fireResistant().rarity(Rarity.RARE).attributes(createAttributes(TIER, 3.5F, -2.6F)));
    ...
    public void onHurt(LivingDamageEvent.@NotNull Pre event) {
        if (event.getSource().getDirectEntity() instanceof LivingEntity attacker && event.getSource().is(DamageTypeTags.IS_PLAYER_ATTACK)) {
            if (attacker.getMainHandItem().is(this) && EnigmaticHandler.isTheCursedOne(attacker)) {
...
//                    cooldowns.addCooldown(EnigmaticItems.THE_CUBE.get(), 400);
    ...
    @SubscribeEvent
    public void onEnderTeleport(EntityTeleportEvent.@NotNull EnderEntity event) {
        ...
        if (killed instanceof EnderMan && killed.getPersistentData().getBoolean("EnderSlayerVictim")) {
            for (ItemEntity entity : event.getDrops()) {
                if (entity.getItem().is(Items.ENDER_PEARL)) {
                    ...
                    this.dropXPOrb(killed.level(), killed.getX(), killed.getY(), killed.getZ(), 8);
                }
            }
